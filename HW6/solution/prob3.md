## Problem 3

We are given the function $f: \mathbb{R}^2 \to \mathbb{R}$:
$$ f(x, y) = \frac{3}{4}x^{\frac{4}{3}} + \frac{1}{2}(x + y)^2 $$

### (a) Find its global minimizer.

To find the minimizer, we first find the critical points by setting the gradient of $f$ to zero.
The gradient is:
$$ \nabla f(x, y) = \begin{pmatrix} \frac{\partial f}{\partial x} \\ \frac{\partial f}{\partial y} \end{pmatrix} = \begin{pmatrix} x^{\frac{1}{3}} + (x+y) \\ x+y \end{pmatrix} $$
Set the gradient to zero:
$$ \begin{pmatrix} x^{\frac{1}{3}} + (x+y) \\ x+y \end{pmatrix} = \begin{pmatrix} 0 \\ 0 \end{pmatrix} $$
From the second equation, we have $x+y=0$.
Substituting this into the first equation gives $x^{1/3} = 0$, which implies $x=0$.
Since $x+y=0$, we have $y = -x = 0$.
The only critical point is $(0, 0)$.

Now, we must determine if this point is a global minimizer.
The function $f(x,y)$ is a sum of two non-negative terms:
- $\frac{3}{4}x^{\frac{4}{3}} = \frac{3}{4}(x^{\frac{1}{3}})^4 \ge 0$ for all $x$.
- $\frac{1}{2}(x+y)^2 \ge 0$ for all $x, y$.
Thus, $f(x, y) \ge 0$ for all $(x, y) \in \mathbb{R}^2$.
At the critical point $(0,0)$, the function value is $f(0,0) = 0$.
Since $f(x,y) \ge f(0,0)$ for all $(x,y)$, the point $(0,0)$ is the global minimizer.

### (b) Newton's method

The update rule for Newton's method is:
$$ \mathbf{x}_{k+1} = \mathbf{x}_k - [\nabla^2 f(\mathbf{x}_k)]^{-1} \nabla f(\mathbf{x}_k) $$
where $\mathbf{x}_k = (x_k, y_k)^T$.

First, we compute the Hessian matrix $\nabla^2 f(x, y)$:
$$ \nabla^2 f(x, y) = \begin{pmatrix} \frac{1}{3}x^{-\frac{2}{3}}+1 & 1 \\ 1 & 1 \end{pmatrix} $$
The inverse of the Hessian is:
$$ [\nabla^2 f(x, y)]^{-1} = \frac{1}{\det(\nabla^2 f)} \begin{pmatrix} 1 & -1 \\ -1 & \frac{1}{3}x^{-\frac{2}{3}}+1 \end{pmatrix} $$
The determinant is $\det(\nabla^2 f) = (\frac{1}{3}x^{-\frac{2}{3}}+1)(1) - (1)(1) = \frac{1}{3}x^{-\frac{2}{3}}$.
The inverse is defined for $x \ne 0$. The problem states we start with an initial point $(x_0, y_0)$ with $x_0 \ne 0$.

Let's compute the Newton step at a point $(x_k, y_k)$ where $x_k \ne 0$:
$$ [\nabla^2 f]^{-1} \nabla f = \left(3x_k^{\frac{2}{3}}\right) \begin{pmatrix} 1 & -1 \\ -1 & \frac{1}{3}x_k^{-\frac{2}{3}}+1 \end{pmatrix} \begin{pmatrix} x_k^{\frac{1}{3}} + x_k+y_k \\ x_k+y_k \end{pmatrix} $$
The top component of the resulting vector is:
$$ 3x_k^{\frac{2}{3}} [ (x_k^{\frac{1}{3}} + x_k+y_k) - (x_k+y_k) ] = 3x_k^{\frac{2}{3}} (x_k^{\frac{1}{3}}) = 3x_k $$
The bottom component is:
$$ 3x_k^{\frac{2}{3}} [ -(x_k^{\frac{1}{3}} + x_k+y_k) + (\frac{1}{3}x_k^{-\frac{2}{3}}+1)(x_k+y_k) ] $$
$$ = 3x_k^{\frac{2}{3}} [ -x_k^{\frac{1}{3}} - (x_k+y_k) + \frac{1}{3}x_k^{-\frac{2}{3}}(x_k+y_k) + (x_k+y_k) ] $$
$$ = 3x_k^{\frac{2}{3}} [ -x_k^{\frac{1}{3}} + \frac{1}{3}x_k^{-\frac{2}{3}}(x_k+y_k) ] = -3x_k + (x_k+y_k) $$
So, the update rule is:
$$ \begin{pmatrix} x_{k+1} \\ y_{k+1} \end{pmatrix} = \begin{pmatrix} x_k \\ y_k \end{pmatrix} - \begin{pmatrix} 3x_k \\ -3x_k + (x_k+y_k) \end{pmatrix} $$
$$ x_{k+1} = x_k - 3x_k = -2x_k $$
$$ y_{k+1} = y_k - (-3x_k + x_k+y_k) = y_k + 3x_k - x_k - y_k = 2x_k $$
The expression for $(x_{k+1}, y_{k+1})$ is $(-2x_k, 2x_k)$.

Now prove that starting from $(x_0, y_0)$ with $x_0 \ne 0$, Newton's method does not converge to the global minimizer.
Let's analyze the sequence of iterates starting from $(x_0, y_0)$ with $x_0 \ne 0$:
- $x_k = (-2)^k x_0$
- $y_k = 2x_{k-1} = 2(-2)^{k-1}x_0 = (-2)^k x_0$ for $k \ge 1$.
The sequence for $x_k$ is $x_0, -2x_0, 4x_0, -8x_0, \dots$.
Since $|-2| > 1$, the magnitude $|x_k| = 2^k |x_0|$ diverges to infinity as $k \to \infty$ (unless $x_0=0$).
Therefore, the sequence of iterates $(x_k, y_k)$ does not converge to the global minimizer $(0,0)$.

### (c) Projected gradient descent

We want to minimize $f$ subject to the constraint $\Omega = \{(x, y) : x + y = 1\}$.
One iteration of the projected gradient descent method is:
1. Compute an intermediate point: $\mathbf{z}_{k+1} = \mathbf{x}_k - \alpha_k \nabla f(\mathbf{x}_k)$.
2. Project the point onto the constraint set: $\mathbf{x}_{k+1} = \text{Proj}_{\Omega}(\mathbf{z}_{k+1})$.

Given a point $\mathbf{x}_k = (x_k, y_k)$ on the line $x_k+y_k=1$.
1. Compute $\mathbf{z}_{k+1} = (z_{k+1,1}, z_{k+1,2})^T$:
   $$ \nabla f(x_k, y_k) = \begin{pmatrix} x_k^{\frac{1}{3}} + (x_k+y_k) \\ x_k+y_k \end{pmatrix} $$
   Since $x_k+y_k=1$, the gradient becomes:
   $$ \nabla f(x_k, y_k) = \begin{pmatrix} x_k^{\frac{1}{3}} + 1 \\ 1 \end{pmatrix} $$
   $$ z_{k+1,1} = x_k - \alpha_k(x_k^{\frac{1}{3}} + 1) $$
   $$ z_{k+1,2} = y_k - \alpha_k(1) $$
2. Project $\mathbf{z}_{k+1}$ onto the line $x+y=1$. The projection of a point $(z_1, z_2)$ onto the line $x+y=c$ is given by $x = \frac{z_1-z_2+c}{2}, y = \frac{-z_1+z_2+c}{2}$. Here $c=1$.
   $$ x_{k+1} = \frac{z_{k+1,1} - z_{k+1,2} + 1}{2} $$
   $$ y_{k+1} = 1 - x_{k+1} $$
Substituting the expressions for $z_{k+1,1}$ and $z_{k+1,2}$:
$$ z_{k+1,1} - z_{k+1,2} = (x_k - \alpha_k(x_k^{\frac{1}{3}} + 1)) - (y_k - \alpha_k) $$
$$ = x_k - y_k - \alpha_k x_k^{\frac{1}{3}} - \alpha_k + \alpha_k = x_k - y_k - \alpha_k x_k^{\frac{1}{3}} $$
Now substitute this into the expression for $x_{k+1}$:
$$ x_{k+1} = \frac{(x_k - y_k - \alpha_k x_k^{\frac{1}{3}}) + 1}{2} $$
Since $(x_k, y_k)$ is on the line, $y_k = 1 - x_k$.
$$ x_{k+1} = \frac{x_k - (1-x_k) - \alpha_k x_k^{\frac{1}{3}} + 1}{2} = \frac{2x_k - 1 - \alpha_k x_k^{\frac{1}{3}} + 1}{2} = x_k - \frac{\alpha_k}{2} x_k^{\frac{1}{3}} $$
And $y_{k+1} = 1 - x_{k+1}$.

So, one iteration is:
$$ x_{k+1} = x_k - \frac{\alpha_k}{2} x_k^{\frac{1}{3}} $$
$$ y_{k+1} = 1 - x_{k+1} $$
This is the expression for one iteration of projected gradient descent.